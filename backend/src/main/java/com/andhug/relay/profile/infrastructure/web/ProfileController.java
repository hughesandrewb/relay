...
public class ProfileController {
  ...
  public List<DirectMessageDto> getDirectMessages(@AuthenticationPrincipal Profile profile) {
    ...
    return directMessageQueryService.getDirectMessages(profile.getId());
  }
  ...
  public DirectMessageDto createDirectMessage(
    ...
    RoomId directMessageId = commandBus.dispatch(createDirectMessageCommand);

    DirectMessageDto directMessage =
        directMessageQueryService.getDirectMessageById(profile.getId(), directMessageId);
  ...
  public ResponseEntity<Void> sendFriendRequest(
      @AuthenticationPrincipal Profile profile, @PathVariable ProfileId addresseeId) {

    var sendFriendRequestCommand = SendFriendRequestCommand.of(profile.getId(), addresseeId);

    commandBus.dispatch(sendFriendRequestCommand);
  ...
  public ResponseEntity<Void> acceptFriend(
      @AuthenticationPrincipal Profile profile, @PathVariable ProfileId requesterId) {
    var acceptFriendRequestCommand = AcceptFriendRequestCommand.of(requesterId, profile.getId());

    commandBus.dispatch(acceptFriendRequestCommand);
