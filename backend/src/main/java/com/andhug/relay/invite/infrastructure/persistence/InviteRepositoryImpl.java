...
public class InviteRepositoryImpl implements InviteRepository {
  ...
  public Invite findById(InviteId inviteId) {
    InviteEntity inviteEntity =
        inviteJpaRepository
            .findById(inviteId.value())
            .orElseThrow(
                () ->
                    new NotFoundException(
                        String.format("Could not find Invite with id: %s", inviteId.toString())));
    ...
    InviteEntity inviteEntity =
        inviteJpaRepository
            ...
            .orElseThrow(
                () ->
                    new NotFoundException(
                        String.format("Could not find Invite with code: %s", code.toString())));
  ...
  public Invite findByWorkspaceIdAndSenderId(WorkspaceId workspaceId, ProfileId senderId) {
    InviteEntity inviteEntity =
        inviteJpaRepository
            .findByWorkspaceIdAndSenderId(workspaceId.value(), senderId.value())
            .orElseThrow(
                () ->
                    new NotFoundException(
                        String.format(
                            "Could not find Invite with WorkspaceId: %s, SenderId: %s",
